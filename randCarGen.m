...
w = .5;
carParams.mass = 177*(w*rand+.5); %170.097; % not including driver
tireParams.p_i = 12*(w*rand+.5); % pressure
DTparams.drivetrain_efficiency = min([0.92*(w*rand+.5) 100]); % copied from old lapsim
DTparams.final_drive = 9.3016*(w*rand+.5);
aeroParams.cda = 0.787*(w*rand+.5);
aeroParams.cla = 2*(w*rand+.5);
aeroParams.distribution = 0.5*(w*rand+.5); % proportion of downforce in front
carParams.weight_dist = 0.51*(w*rand+.5); % percentage of weight in rear
